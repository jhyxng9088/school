...
export function TodoPage({ now, todoData }) {
  ...
  const naturalResult = useMemo(() => parseReminderText(naturalText, now), [naturalText, now])
  ...
  useLayoutEffect(() => {
    ...
    // Never let the state/FLIP motion replay the initial page entrance.
    if (pageEntering) {
    ...
    // First stable frame after page entrance: seed positions only.
