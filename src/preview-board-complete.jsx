...
import './preview-board-theme.css'
...
function PostBadges({ post, sections }) {
  ...
  return (
    <>
      ...
      {post.kind === 'question' && post.resolved ? <span className="preview-board-badge is-resolved">해결됨</span> : null}
    </>
  )
}
...
function BoardSectionComposer({ open, sections, onClose, onCreated }) {
  ...
  const availableColors = useMemo(() => REMINDER_CATEGORY_COLORS.filter((item) => !usedColors.has(item.id)), [usedColors])
    ...
    >
      <form className="preview-board-section-form" onSubmit={submit}>
        ...
        <div className="preview-board-section-actions">
          ...
          <button type="submit" className="is-primary" disabled={!canCreate}>{pending ? '추가 중…' : '추가'}</button>
        </div>
      </form>
    </UnifiedBottomSheet>
  )
...
function BoardComposer({ open, sections, initialSectionId, onClose, onCreated }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      setProgress(files.length ? '게시글을 저장하는 중…' : '')
  ...
  return (
    <UnifiedBottomSheet open={open} onClose={onClose} title="새 게시글" subtitle="같은 반 친구들에게만 보여요." ariaLabel="새 게시글 작성" closeDisabled={pending}>
      <form className="preview-board-form" onSubmit={submit}>
        ...
        <label className="preview-board-field">
          <span>제목</span>
          <input type="text" value={title} maxLength={70} placeholder="제목을 입력해 주세요" onChange={(event) => setTitle(event.target.value)} disabled={pending} />
          <small className="preview-board-field-count">{title.length}/70</small>
        </label>
        <label className="preview-board-field">
          <span>내용</span>
          <textarea value={body} maxLength={1200} rows={6} placeholder="내용을 입력해 주세요" onChange={(event) => setBody(event.target.value)} disabled={pending} />
          <small className="preview-board-field-count">{body.length}/1200</small>
        </label>
        <BoardAttachmentPicker files={files} onChange={setFiles} onError={setError} disabled={pending} />
        {progress ? <p className="preview-board-upload-progress" role="status">{progress}</p> : null}
        {error ? <p className="preview-board-error" role="alert">{error}</p> : null}
        <div className="preview-board-sheet-actions">
          <button type="button" onClick={onClose} disabled={pending}>취소</button>
          <button type="submit" className="is-primary" disabled={!canPublish}>{pending ? '올리는 중…' : '게시하기'}</button>
        </div>
      </form>
    </UnifiedBottomSheet>
  )
...
function BoardPostEditor({ post, sections, open, onClose, onUpdated, onDeleted }) {
  ...
  if (!post) return null
  const existingAttachments = (Array.isArray(post.attachments) ? post.attachments : []).filter((item) => keptAttachmentIds.includes(item.id))
  const remainingSlots = Math.max(0, BOARD_ATTACHMENT_LIMIT - existingAttachments.length)
  const canSave = title.trim().length > 0 && body.trim().length > 0 && !pending && !deleting && existingAttachments.length + files.length <= BOARD_ATTACHMENT_LIMIT
  ...
  return (
    <UnifiedBottomSheet open={open} onClose={onClose} title="게시글 수정" subtitle="내용과 섹션, 첨부 파일을 함께 수정할 수 있어요." ariaLabel="게시글 수정" closeDisabled={pending || deleting}>
      <form className="preview-board-form preview-board-edit-form" onSubmit={submit}>
        <div className="preview-board-compose-sections" role="group" aria-label="게시할 섹션">
          {sections.map((section) => (
            <button type="button" key={section.id} className={sectionId === section.id ? 'is-active' : ''} aria-pressed={sectionId === section.id} onClick={() => setSectionId(section.id)} disabled={pending || deleting}>
              <SectionDot section={section} /><span>{section.label}</span>
            </button>
          ))}
        </div>
        <label className="preview-board-field">
          ...
          <section className="preview-board-existing-attachments" aria-label="현재 첨부 파일">
            ...
            {existingAttachments.map((attachment) => (
              <div className="preview-board-existing-file" key={attachment.id}>
                ...
                <button type="button" onClick={() => setKeptAttachmentIds((current) => current.filter((id) => id !== attachment.id))} disabled={pending || deleting}>제거</button>
              </div>
            ))}
          </section>
        ) : null}
        <BoardAttachmentPicker files={files} onChange={setFiles} onError={setError} disabled={pending || deleting} maxFiles={remainingSlots} />
        {progress ? <p className="preview-board-upload-progress" role="status">{progress}</p> : null}
        {error ? <p className="preview-board-error" role="alert">{error}</p> : null}
        <div className="preview-board-danger-zone">
          ...
          <div>
            ...
            <button type="button" className="is-danger" onClick={removePost} disabled={pending || deleting}>{deleting ? '삭제 중…' : deleteArmed ? '정말 삭제' : '게시글 삭제'}</button>
          </div>
        </div>
        <div className="preview-board-sheet-actions">
...
function BoardSectionEditor({ section, sections, open, onClose, onUpdated, onDeleted }) {
  ...
  const canSave = label.trim().length > 0 && Boolean(color) && !pending && !deleting
  ...
  return (
    <UnifiedBottomSheet open={open} onClose={onClose} title="섹션 편집" subtitle="직접 만든 섹션의 이름과 색상을 바꿀 수 있어요." ariaLabel="게시판 섹션 편집" closeDisabled={pending || deleting} className="preview-board-section-sheet">
      <form className="preview-board-section-form" onSubmit={submit}>
        <label className="preview-board-section-name-field">
          <span>섹션 이름</span>
        ...
        <fieldset className="preview-board-section-colors">
          ...
          <div>
            {REMINDER_CATEGORY_COLORS.map((item) => {
              const used = usedColors.has(item.id)
              return (
                <button type="button" className={color === item.id ? 'is-selected' : ''} aria-label={`${item.label}${used ? ', 사용 중' : ''}`} aria-pressed={color === item.id} disabled={used || pending || deleting} onClick={() => setColor(item.id)} key={item.id}>
                  <span style={{ '--preview-board-section-color': item.id }} aria-hidden="true" />
                </button>
              )
            })}
        ...
        <div className="preview-board-danger-zone">
          ...
          <div>
            {deleteArmed ? <button type="button" onClick={() => setDeleteArmed(false)} disabled={deleting}>취소</button> : null}
            <button type="button" className="is-danger" onClick={removeSection} disabled={pending || deleting}>{deleting ? '삭제 중…' : deleteArmed ? '섹션 삭제 확인' : '섹션 삭제'}</button>
          </div>
        </div>
        <div className="preview-board-section-actions">
          <button type="button" onClick={onClose} disabled={pending || deleting}>닫기</button>
...
function BoardDetail({ post, sections, meKey, open, onClose, onUpdated, onEditPost, onMutated }) {
  ...
  return (
    <UnifiedBottomSheet open={open} onClose={onClose} title={post.title} subtitle={`${post.authorName || '학생'} · ${formatBoardTime(post.createdAt)}`} ariaLabel="게시글 상세" closeDisabled={actionPending}>
      <div className="preview-board-detail">
        ...
        {canResolve ? <button type="button" className="preview-board-resolve" onClick={resolveQuestion} disabled={resolvePending}>{resolvePending ? '처리 중…' : '질문 해결로 표시'}</button> : null}
  ...
  return (
    <section className="preview-board-page">
      <header className="preview-board-header">
        ...
        <button type="button" className="preview-board-write" onClick={() => setComposerOpen(true)}><PlusIcon /><span>글쓰기</span></button>
      </header>
      <BoardSections sections={sections} activeSectionId={activeSectionId} onSelect={selectSection} onAdd={() => setSectionComposerOpen(true)} />
      <div key={activeSectionId} className="preview-board-section-view" style={{ '--board-section-enter-x': `${sectionDirection * 9}px` }}>
        <div className="preview-board-toolbar">
          ...
          <div className="preview-board-toolbar-actions">
            ...
            <button type="button" className="preview-board-refresh" onClick={() => refresh()} disabled={refreshing || loading} aria-label="게시판 새로고침">{refreshing ? '불러오는 중' : '새로고침'}</button>
          </div>
        </div>
        {hasPosts ? (
          <>
            ...
            {hasMore ? <button type="button" className="preview-board-load-more" onClick={loadMore} disabled={loadingMore}>{loadingMore ? '이전 글 불러오는 중…' : '이전 글 더 보기'}</button> : null}
          </>
      ...
      <BoardPostEditor post={postEditor} sections={sections} open={Boolean(postEditorId && postEditor)} onClose={() => setPostEditorId('')} onUpdated={handlePostEdited} onDeleted={handlePostDeleted} />
    </section>
  )
}
