...
export function normalizeWeeklyScheduleShape(value) {
...
export function applyManualWeeklyOverrides(baseSchedule, manualOverrides) {
  const merged = normalizeWeeklyScheduleShape(baseSchedule)
...
export function reconcileManualWeeklyOverrides(previousBase, previousManual, currentEffective) {
  const base = normalizeWeeklyScheduleShape(previousBase)
  const manual = normalizeManualWeeklyOverrides(previousManual)
  const current = normalizeWeeklyScheduleShape(currentEffective)
...
export function manualOverridesFromDifference(baseSchedule, effectiveSchedule) {
  const base = normalizeWeeklyScheduleShape(baseSchedule)
  const effective = normalizeWeeklyScheduleShape(effectiveSchedule)
  const manual = {}
...
export function buildNeisTimetableSyncState({
  ...
  const nextBase = normalizeWeeklyScheduleShape(neisWeeklySchedule)
  const hasCurrentEffective = data.weeklySchedule && typeof data.weeklySchedule === 'object'
  const hasPreviousBase = data.neisWeeklySchedule && typeof data.neisWeeklySchedule === 'object'

  let manual = normalizeManualWeeklyOverrides(data.manualWeeklyOverrides)

  if (hasPreviousBase && hasCurrentEffective) {
    manual = reconcileManualWeeklyOverrides(
      data.neisWeeklySchedule,
      manual,
      data.weeklySchedule,
    )
  } else if (hasCurrentEffective) {
    const documentUpdatedAt = Number(data.updatedAt || 0)
    ...
    if (legacyWasEditedAfterLastSync) {
      manual = manualOverridesFromDifference(nextBase, data.weeklySchedule)
    }
  }
