...
function safeReminderStateId(todoId) {
  ...
  return `${INTERNAL_PREFIX}summary_${safe}`
}
...
async function startUnreadIndicators() {
  ...
  function hasUnreadSummary() {
    for (const todo of state.todos.values()) {
      if (summaryUnread(todo)) return true
    }
    return false
  }
  ...
  function navUnread(tab) {
    if (tab === 'todo') {
      return otherActivityVersion('reminder') > seenVersion(NAV_STATE_IDS.todo) || hasUnreadSummary()
    }
    if (tab === 'timetable') {
  ...
  function renderReminderRows() {
    document.querySelectorAll('.todo-stage5 [data-reminder-id]').forEach((row) => {
      ...
      if (summaryUnread(todo)) addDot(row, 'reminder')
      else removeDot(row)
    })
  }
  ...
  function waitForSummaryOpen(todo) {
    if (!todo || !summaryUnread(todo)) return
    let done = false
    let observer = null
    let timeout = 0
...
function boot() {
  startUnreadIndicators().then((started) => {
    ...
    console.error('Unread indicators failed to start:', error)
    startAttempts += 1
