...
function replacePreviewSource(source, id) {
  ...
  next = patchPreviewNavStabilitySource(next, cleanId)
  next = patchPreviewNavRealMotionSource(next, cleanId)
  return next
}
  ...
  return {
    ...
    transform(code, id) {
      ...
      if (
        ...
        || cleanId.endsWith('/preview-nav-stability-patch.js')
        || cleanId.endsWith('/preview-nav-real-motion-patch.js')
      ) return null
