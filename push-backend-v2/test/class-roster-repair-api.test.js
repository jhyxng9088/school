...
test('roster repair keeps its public URL while sharing the authenticated roster function', () => {
  ...
  assert.match(apiSource, /const mode = String\(req\.query\?\.mode \|\| ''\)\.trim\(\)/)
  assert.match(apiSource, /const repairMode = mode === 'repair'/)
...
test('reminder-section route accepts authenticated production and preview classes and preserves retry-safe restore provenance', () => {
  ...
  assert.match(apiSource, /const mode = String\(req\.query\?\.mode \|\| ''\)\.trim\(\)/)
  assert.match(apiSource, /const reminderSectionMode = mode === 'reminder-sections'/)
