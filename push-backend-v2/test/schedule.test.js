...
test('알림 문구 형식', () => {
  assert.equal(reminderHourBody('수학 과제'), '수학 과제 관련 내용을 확인해 주세요.')
  assert.equal(reminderTomorrowBody([{ title: '수학 과제' }]), '내일 수학 과제 있습니다. 확인해 주세요.')
  assert.equal(reminderTomorrowBody([{ title: '수학 과제' }, { title: '영어 단어' }]), '내일 수학 과제 외 1개의 할 일이 있습니다. 확인해 주세요.')
  assert.equal(academicTomorrowBody([{ title: '체육대회' }]), '내일 체육대회 예정입니다.')
  assert.equal(academicTomorrowBody([{ title: '체육대회' }, { title: '동아리 발표' }]), '내일 체육대회 외 1개의 중요 일정이 있습니다.')
})

test('알림 제목 앞 목록 기호는 제거하고 제목 내부 하이픈은 유지한다', () => {
  assert.equal(reminderHourBody('-수학 과제'), '수학 과제 관련 내용을 확인해 주세요.')
  assert.equal(reminderTomorrowBody([{ title: '— 실행' }]), '내일 실행 있습니다. 확인해 주세요.')
  assert.equal(academicTomorrowBody([{ title: '-실행' }]), '내일 실행 예정입니다.')
  assert.equal(academicTomorrowBody([{ title: 'AI-반도체 발표' }]), '내일 AI-반도체 발표 예정입니다.')
})
...
test('1시간 전 알림은 완료/숨김 학생을 빼고 미완료 학생 기기에만 보낸다', () => {
  ...
  assert.equal(plans[0].payload.body, '수학 과제 관련 내용을 확인해 주세요.')
})
...
test('23시 알림은 학생별 미완료 리마인더만 묶는다', () => {
  ...
  assert.equal(reminders.find((p) => p.studentKey === 'student-a').payload.body, '내일 수학 과제 있습니다. 확인해 주세요.')
  assert.equal(reminders.find((p) => p.studentKey === 'student-b').payload.body, '내일 수학 과제 외 1개의 할 일이 있습니다. 확인해 주세요.')
})
...
test('학사일정은 중요 토글 prefix가 있는 일정만 전날 23시에 보낸다', () => {
  ...
  assert.equal(academic.every((plan) => plan.payload.body === '내일 체육대회 예정입니다.'), true)
