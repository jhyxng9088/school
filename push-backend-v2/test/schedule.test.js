...
test('알림 문구 형식', () => {
  ...
  assert.equal(reminderTomorrowBody([{ title: '수학 과제' }]), '내일 수학 과제가 있어요.')
  assert.equal(reminderTomorrowBody([{ title: '수학 과제' }, { title: '영어 단어' }]), '내일 수학 과제 외 1개의 할 일이 있어요.')
...
test('1시간 전 알림은 완료/숨김 학생을 빼고 미완료 학생 기기에만 보낸다', () => {
  ...
  assert.equal(plans[0].payload.body, '수학 과제 확인해 주세요.')
})
...
test('23시 알림은 학생별 미완료 리마인더만 묶는다', () => {
  ...
  assert.equal(reminders.find((p) => p.studentKey === 'student-a').payload.body, '내일 수학 과제가 있어요.')
  assert.equal(reminders.find((p) => p.studentKey === 'student-b').payload.body, '내일 수학 과제 외 1개의 할 일이 있어요.')
})
