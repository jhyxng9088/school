...
async function optional(getter, fallback) {
  try { return await getter() } catch (error) {
    console.warn('admin overview v3 optional query skipped', { code: error?.code, message: error?.message })
    return fallback
  }
}
...
async function loadClassData(db, classId, now) {
  const classRef = db.collection('classes').doc(classId)
  const [presenceSnap, activitySnap, reminders, academics, timetableSnap] = await Promise.all([
    classRef.collection('presence').where('lastSeenMs', '>=', now - MONTH_WINDOW_MS).get(),
    classRef.collection('activity').where('updatedAt', '>=', now - MONTH_WINDOW_MS).get(),
    countCollection(classRef.collection('todos')),
    countCollection(classRef.collection('academicEvents')),
    optional(() => classRef.collection('settings').doc('timetable').get(), null),
  ])

  const timetable = timetableSnap?.exists ? timetableSnap.data() || {} : {}
  return {
    classId,
    presence: (presenceSnap.docs || []).map((item) => ({ id: item.id, ...(item.data() || {}) })),
    activity: (activitySnap.docs || []).map((item) => ({ id: item.id, classId, ...(item.data() || {}) })),
    reminders,
    academics,
    ...
    timetableOverrideCount: timetable.overrides && typeof timetable.overrides === 'object'
      ? Object.keys(timetable.overrides).length
      : 0,
  }
}
...
async function buildOverview() {
  ...
  const [usersSnap, devicesSnap, pushSnap, auditSnap, authMap] = await Promise.all([
  ...
  const classIds = [...new Set(
    (usersSnap.docs || [])
      .map((snapshot) => safe(snapshot.data()?.classId, 40))
      .filter((value) => /^class-\d{1,2}$/.test(value)),
  )]

  const classData = await Promise.all(classIds.map((classId) => loadClassData(db, classId, now)))
  ...
  for (const row of classData) {
    classMeta.set(row.classId, row)
    for (const item of row.presence) {
      const key = safe(item.studentKey, 100)
  ...
  activity.sort((a, b) => Number(b.updatedAt || 0) - Number(a.updatedAt || 0))
  ...
  const users = [...grouped.entries()].map(([studentKey, identities]) => {
    ...
    const device = registeredDevice || pushDeviceByStudent.get(studentKey) || {
      deviceType: 'unknown',
      deviceLabel: '미수집',
      browser: '',
      displayMode: '',
  ...
  for (const user of users) {
    ...
    if (!classesMap.has(key)) {
      const meta = classMeta.get(key) || {
        reminders: 0,
        academics: 0,
        timetableConfigured: false,
        timetableUpdatedAt: 0,
        timetableOverrideCount: 0,
      }
  ...
  const classes = [...classesMap.values()].sort((a, b) => (a.grade ?? 99) - (b.grade ?? 99)
    || (a.classNumber ?? 99) - (b.classNumber ?? 99))
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error('admin overview v3 failed', { code: error?.code, message: error?.message })
    const status = Number(error?.status || 500)
    return res.status(status).json({
      ok: false,
      error: safe(error?.code, 80) || 'overview_failed',
      message: safe(error?.message, 160) || '관리자 데이터를 불러오지 못했어.',
    })
  }
}
