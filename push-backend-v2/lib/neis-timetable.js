...
async function neisJson(path, params) {
...
export async function getClassTimetable(classNumber, fromDate, toDate) {
  ...
  return rows.map((row) => ({
    ...
    classNumber: Number(row.CLASS_NM || number),
...
export function weeklyScheduleFromRows(rows) {
  ...
  for (const row of rows || []) {
    ...
    if (!key || !Number.isInteger(row.period) || row.period < 1 || row.period > 10) continue
    weeklySchedule[key][row.period] = String(row.subject || '').trim()
  }
  return weeklySchedule
}

export const NEIS_TIMETABLE_META = { officeCode: OFFICE_CODE, schoolName: SCHOOL_NAME, grade: GRADE }
