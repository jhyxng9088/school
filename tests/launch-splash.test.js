...
test('configured launch mounts canonical owners immediately but reveals only after a stable Home paint, including holiday layout', () => {
  ...
  assert.match(main, /const launchHomeReady = presence\?\.ready === true[\s\S]*homeSignalsReady === true[\s\S]*studyStatusLaunchReady === true[\s\S]*todoData\.ready === true[\s\S]*timetableLaunchReady === true/)
