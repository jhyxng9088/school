...
test('study class label is source-owned and its recovery owner is retired', () => {
  ...
  assert.doesNotMatch(recovery, /patchStudyClassLabel/)
  assert.doesNotMatch(recovery, /production study class label/)
...
test('study ranking click owner is direct and keeps vertical scrolling available', () => {
  ...
  assert.doesNotMatch(previewPage, /onPointerDown=/)
  assert.doesNotMatch(previewPage, /touchIntentRef/)
  assert.doesNotMatch(previewPage, /performance\.now\(\) - intent\.at/)
  assert.match(previewPage, /onClick=\{\(\) => selectScope\('class'\)\}/)
  assert.match(previewPage, /onClick=\{\(\) => selectScope\('school'\)\}/)
  assert.match(previewPage, /useStudyRankingScopeSpring/)
  assert.match(previewPage, /data-direction=\{stageDirection\}/)
  assert.doesNotMatch(recovery, /patchStudyRankingGesture/)
  assert.doesNotMatch(recovery, /replaceLegacyOrCanonical/)
  assert.doesNotMatch(recovery, /endsWith\('\/src\/preview-study\.jsx'\)/)
