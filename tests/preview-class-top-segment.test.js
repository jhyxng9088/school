...
test('class navigation uses the same canonical top-segment spring owner as schedule', () => {
  ...
  assert.doesNotMatch(component, /useSHubSegmentSpring|deform:/)
...
test('top segment emits the canonical shared spring wrapper directly', () => {
  ...
  const springStart = source.indexOf('function useClassTopSegmentSpring(activeIndex, { shellElastic = true } = {}) {')
