...
test('AI compose entry cascades through visible blocks instead of moving as one stage', () => {
  ...
  assert.match(css, /animation: s-hub-ai-piece-in 690ms cubic-bezier\(0\.16, 1, 0\.3, 1\) both/)
  assert.match(css, /\.s-hub-ai-page-hero \{ animation-delay: 30ms; \}/)
  assert.match(css, /capability:nth-child\(1\) \{ animation-delay: 105ms; \}/)
  assert.match(css, /capability:nth-child\(2\) \{ animation-delay: 155ms; \}/)
  assert.match(css, /capability:nth-child\(3\) \{ animation-delay: 205ms; \}/)
  assert.match(css, /section:nth-child\(1\) > \.s-hub-ai-page-extra-head \{ animation-delay: 280ms; \}/)
  assert.match(css, /quick:nth-child\(1\) \{ animation-delay: 355ms; \}/)
  assert.match(css, /quick:nth-child\(4\) \{ animation-delay: 505ms; \}/)
  assert.match(css, /section:nth-child\(2\) > \.s-hub-ai-page-extra-head \{ animation-delay: 580ms; \}/)
  assert.match(css, /context-item:nth-child\(1\) \{ animation-delay: 655ms; \}/)
  assert.match(css, /context-item:nth-child\(4\) \{ animation-delay: 805ms; \}/)
  assert.match(css, /> \.s-hub-ai-content \{ animation-delay: 880ms; \}/)
})
...
test('AI working and result states keep the existing calm state transition', () => {
  ...
  assert.match(css, /html\.school-mobile-compat \.s-hub-ai-page-hero,[\s\S]*animation-duration: 600ms/)
