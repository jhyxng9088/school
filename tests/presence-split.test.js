...
test('member total is cached and cannot block the active presence transport', () => {
  ...
  assert.doesNotMatch(source, /await ensureMemberBestEffort\(\)/)
  const memberRegistration = source.indexOf('void ensureMemberBestEffort()')
  const supabaseStart = source.indexOf('supabasePresence = startSupabasePresence({')
  assert.ok(memberRegistration >= 0 && supabaseStart > memberRegistration)
