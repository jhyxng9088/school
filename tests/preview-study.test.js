...
test('study start enters the active timer locally before the server request completes', () => {
  ...
  assert.match(page, /const myActive = optimisticActive \|\| serverActive/)
...
test('optimistic Study start rolls back safely on failure and serializes later controls', () => {
  const page = read('src/preview-study.jsx')
  const start = page.slice(page.indexOf('async function start()'), page.indexOf('async function pause()'))
  ...
  assert.match(start, /setCustomSubject\(previousCustomSubject\)/)
  assert.match(start, /setActionError\(error\?\.message \|\| '공부를 시작하지 못했습니다\.'\)/)
  assert.match(wait, /const pending = startRequestRef\.current/)
  assert.match(wait, /if \(!pending\) return true/)
  assert.match(wait, /return pending/)
  assert.match(pause, /await waitForPendingStart\(\)/)
  assert.match(resume, /await waitForPendingStart\(\)/)
  assert.match(stop, /await waitForPendingStart\(\)/)
  assert.doesNotMatch(start, /localStorage|indexedDB|addEventListener/)
})
