...
test('current Study rows reuse the reminder enter and collapse motion owners', () => {
  ...
  assert.match(studyStyle, /animation: reminder-row-mount 440ms cubic-bezier\(\.16, 1, \.3, 1\) both/)
  assert.match(studyStyle, /\.preview-study-live-person\.is-state-leaving/)
  assert.match(studyStyle, /animation: reminder-row-collapse 280ms cubic-bezier\(\.4, 0, 1, 1\) both !important/)
