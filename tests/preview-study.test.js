...
test('study ranking separates class and school and opens a student detail sheet', () => {
  ...
  assert.match(page, /<UnifiedBottomSheet/)
  assert.match(page, /ariaLabel=\{student\.name \+ ' 공부 기록'\}/)
...
test('current Study rows reuse the reminder enter and collapse motion owners', () => {
  const page = read('src/preview-study.jsx')
  const studyStyle = read('src/preview-study.css')
  const todoStyle = read('src/todo.css')
  const reminderStyle = read('src/todo-stage5.css')

  assert.match(todoStyle, /@keyframes reminder-row-mount/)
  assert.match(todoStyle, /@keyframes reminder-row-collapse/)
  assert.match(todoStyle, /--reminder-row-max-height/)
  assert.match(todoStyle, /--reminder-row-min-height/)
  assert.doesNotMatch(reminderStyle, /@keyframes reminder-row-mount/)
  assert.doesNotMatch(reminderStyle, /@keyframes reminder-row-collapse/)
  assert.match(reminderStyle, /animation: reminder-row-mount 440ms/)
  assert.match(reminderStyle, /animation: reminder-row-collapse 280ms/)
  assert.match(studyStyle, /reminder-row-mount 440ms cubic-bezier\(\.16, 1, \.3, 1\) both/)
  assert.match(studyStyle, /preview-study-live-row-expand 360ms cubic-bezier\(\.16, 1, \.3, 1\) both/)
  assert.match(studyStyle, /\.preview-study-live-person\.is-state-leaving/)
  assert.match(studyStyle, /reminder-row-collapse 280ms cubic-bezier\(\.4, 0, 1, 1\) both/)
  assert.match(studyStyle, /preview-study-live-row-padding-collapse 280ms cubic-bezier\(\.4, 0, 1, 1\) both !important/)
  assert.match(studyStyle, /\.preview-study-live-list \{[\s\S]*min-height: 62px/)
  assert.match(studyStyle, /\.preview-study-live-empty \{[\s\S]*min-height: 62px/)
  assert.match(page, /const \[renderedActive, setRenderedActive\] = useState\(active\)/)
  assert.match(page, /const \[leavingIds, setLeavingIds\] = useState\(\(\) => new Set\(\)\)/)
  assert.match(page, /useLayoutEffect\(\(\) => \{[\s\S]*previousActiveRef\.current = active/)
  assert.match(page, /event\.animationName !== 'reminder-row-collapse'/)
  assert.match(page, /preview-study-live-list\$\{renderedActive\.length \? '' : ' is-empty'\}/)
  assert.match(page, /preview-study-live-empty/)
  assert.match(page, /onAnimationEnd=\{\(event\) => finishRowExit\(id, event\)\}/)
...
test('study start enters the active timer locally before the server request completes', () => {
  ...
  assert.match(start, /const optimisticActive = \{/)
  assert.match(start, /segmentStartedAt: startedAt/)
  assert.match(start, /setOptimisticActive\(optimisticActive\)/)
  assert.match(start, /patchPreviewStudyActiveCache\(optimisticActive\)/)
  assert.ok(start.indexOf('setOptimisticActive(optimisticActive)') < start.indexOf('await startPreviewStudy(subject)'))
