...
test('board originals reuse reminder save behavior and load lazily', () => {
  ...
  assert.match(sharedViewer, /navigator\.canShare\?\.\(\{ files: \[file\] \}\)/)
  assert.match(sharedViewer, /await navigator\.share\(\{ files: \[file\]/)
  assert.match(sharedViewer, /anchor\.download = original\.name \|\| '원본-파일'/)
  assert.match(sharedViewer, /savingRef\.current = true/)
  assert.match(sharedViewer, /DOWNLOAD_GESTURE_LOCK_MS = 700/)
  assert.match(sharedViewer, /원본 저장/)
