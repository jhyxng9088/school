...
test('class-scoped section overrides use the production endpoint for authenticated production and preview classes', () => {
  ...
  assert.match(classApi, /const mode = String\(req\.query\?\.mode \|\| ''\)\.trim\(\)/)
  assert.match(classApi, /if \(mode === 'v2'\) return handleClassRosterV2\(req, res\)/)
  assert.match(classApi, /const reminderSectionMode = mode === 'reminder-sections'/)
