...
test('home cards use semantic V2 destinations through React-owned navigation', () => {
  ...
  assert.match(main, /function TimetablePreview\([\s\S]*className="home-detail-card-action"[\s\S]*onClick=\{\(\) => onNavigate\?\.\('timetable'\)\}/)
  ...
  assert.match(todo, /todo-home-preview" data-home-nav-ready="true"/)
